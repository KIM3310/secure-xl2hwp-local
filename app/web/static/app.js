...
const els = {
  ...
  workflowLaunchpadSteps: document.getElementById("workflowLaunchpadSteps"),
...
function renderReviewPack(payload = null) {
  ...
  const handoffText = handoffBundle.ready_for_handoff
    ? "verify route ready"
...
function renderWorkflowLaunchpad() {
  ...
  const gate = bootstrapRequired
    ? "bootstrap required"
    : hasUser
      ? (resultReady ? "signed handoff evidence open" : "operator logged in")
      : "login required";
  const status = bootstrapRequired
    ? "bootstrap-first"
    : hasUser
      ? (resultReady ? "signed handoff ready" : "operator login active")
      : "login-first";
  const nextStep = bootstrapRequired
    ? "Create the first admin account."
    : hasUser
      ? (resultReady ? "Copy signed handoff or verify snapshot." : `Run one ${state.mode === "file" ? "file" : "path"} job.`)
      : "Sign in with a local operator account.";
  ...
  const steps = bootstrapRequired
    ...
    : hasUser
      ? [
          `Signed-in operator: ${state.user.user_id} (${state.user.role}).`,
          resultReady
      ...
      : [
          ...
          `Use ${state.mode === "file" ? "File Mode" : "Path Mode"} for a single representative run instead of a broad demo.`,
  ...
  if (els.workflowLaunchpadNextStep) els.workflowLaunchpadNextStep.textContent = nextStep;
  renderBriefList(els.workflowLaunchpadSteps, steps);
