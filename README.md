...
## 포트폴리오 포지셔닝
- 이 저장소는 공용 SaaS가 아니라 폐쇄망/로컬 보안 환경을 전제로 한 자동화 워크플로우입니다.
- 신뢰 포인트는 `auth`, `readiness`, `ops`, `export signing` 표면과 실제 오프라인 설치/검증 흐름에 있습니다.


## 커리어 시그널
- **AI 엔지니어 관점:** 계약 기반 정제, 로컬 모델 운용, 결정론적 fallback이 함께 보입니다.
- **솔루션/클라우드 아키텍트 관점:** auth, readiness, ops, export signing이 분리된 폐쇄망 워크플로우를 설명할 수 있습니다.
- **프리세일즈/세일즈 관점:** 보안 환경에서도 재현 가능한 처리·감사·내보내기 story가 분명합니다.
