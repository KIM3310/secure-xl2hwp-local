...
def test_ui_home_page() -> None:
    ...
    assert "workflowLaunchpadSteps" in response.text
    assert "workflowLaunchpadFallback" in response.text
...
def test_health_includes_auth_bootstrap_state() -> None:
    ...
    assert payload["links"]["template_drift_preview"] == "/ops/template-drift-preview"
    assert payload["links"]["export_verification_pack"] == "/ops/export-verification-pack"
    assert payload["links"]["offline_deployment_pack"] == "/ops/offline-deployment-pack"
...
def test_service_brief_and_process_schema_shape() -> None:
    ...
    assert any(item["path"] == "/ops/export-verification-pack" for item in brief_payload["proof_assets"])
    assert any(item["path"] == "/ops/offline-deployment-pack" for item in brief_payload["proof_assets"])
    ...
    assert (
        review_payload["proof_bundle"]["offline_deployment_contract"]
        == "secure-xl2hwp-offline-deployment-pack-v1"
    )
    ...
    verification_response = client.get("/ops/export-verification-pack")
    assert verification_response.status_code == 200
    verification_payload = verification_response.json()
    assert verification_payload["schema"] == "secure-xl2hwp-export-verification-pack-v1"
    assert verification_payload["links"]["export_verification_pack"] == "/ops/export-verification-pack"
    assert verification_payload["verification_contract"]["verify_route"] == "/ops/audit/export/verify"

    deployment_response = client.get("/ops/offline-deployment-pack")
    ...
    assert drift_payload["schema"] == "secure-xl2hwp-template-drift-preview-v1"
    assert drift_payload["summary"]["detected_placeholders"] >= 1
    assert drift_payload["links"]["template_drift_preview"] == "/ops/template-drift-preview"
